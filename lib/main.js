 ...
 * @param {Object} config
 * @return {FileFog} for chaining
 * @api public
 */
FileFog.prototype.use = function(name, provider_classes,config) {

    if(!provider_classes || !provider_classes.provider || !provider_classes.transform || !provider_classes.client){
        throw new Exception("Could not register provider, it is invalid.")
    }
    else if(!name){
 ...
 * @api public
 */
FileFog.prototype.provider = function(name) {
    ...
    return ProviderWrapper(definition.classes.provider, name, definition.config);
};
